function [q, qd, qdd, torque] = simulateRobo(robot, controlFunc, q_desired, t)
    % Simulate a constant robot
    % controlFunc - function of the form:
    %     torque = controlFunc(t_now, q_desired, q_now)
    %     where torque, q_desired and q_now are row vectors; torque has
    %     length NUM_JOINTS
    ...
    for iter = 2:NUM_ITER
        ...
        torque(iter-1,:) = controlFunc(t_now, q_desired(iter-1,:), q_now);
