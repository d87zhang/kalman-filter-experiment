function [q, qd, qdd] = quinticSpline(q_spec, qd_spec, qdd_spec, t_sites, t)
    ...
    % Boundary conditions for each piece-wise polynomial is given in by
    % the q_spec, qd_spec and qdd_spec matrices where (for e.g.)
    %   for any joint index j:
    %   let k be such that t(k) = t_sites(i)
    %   q(k, j) = q_spec(i, j), for all i = 1:length(t_sites)
